...
def init(c):
    ...
    for name in ('admin', 'librarian', 'member'):
        ...
        c.run(f'echo "from django.contrib.auth import get_user_model; User = get_user_model(); User.objects.{function}(\'{name}\', \'{email}\', \'{password}\')" | python manage.py shell')

    # Make librarian staff and add catalog permissions
    c.run('''python manage.py shell -c "
